...
# Header
svg.append(f'<text x="{MARGIN}" y="40" font-size="22" font-weight="700" fill="#1a202c">Dimple normalisation — FrameCAD ORIGINAL vs NORMALISED (rules: 15mm margin, 400mm max gap)</text>')
svg.append(f'<text x="{MARGIN}" y="62" font-size="14" fill="#dc2626" font-weight="600">RED row = ORIGINAL FrameCAD output (the BEFORE — these positions FAIL the 15mm/400mm rules)</text>')
...
def draw_row(x, y, w, length_mm, dimples, is_orig, label_left):
    ...
    if dimples:
        ...
        for i in range(len(dimples) - 1):
            ...
            gap_col = '#dc2626' if (gap > 400 and is_orig) else txt_col
            out.append(f'<text x="{dx_mid}" y="{y - 6}" text-anchor="middle" font-size="10" font-weight="700" fill="{gap_col}">{gap:.0f}mm</text>')
            if gap > 400 and is_orig:
                out.append(f'<text x="{dx_mid}" y="{y - 18}" text-anchor="middle" font-size="9" font-weight="700" fill="#dc2626">FAIL (>400)</text>')
    return out
...
svg.append(f'<text x="{MARGIN + 15}" y="{foot_y + 10}" font-size="13" fill="#14532d"><tspan font-weight="700">Result:</tspan> all 10 Box pieces in this job normalised. Every margin is now exactly 15mm. Every gap is &lt;= 400mm.</text>')

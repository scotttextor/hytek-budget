"""3D Truss V3 - WEB HOLES DRILLED THROUGH at every intersection.

Key fixes:
  - Web holes are now BLACK CYLINDERS piercing through the steel,
    clearly visible from any angle
  - 3 holes per junction in the WEB HOLE pattern (3 × Ø3.8mm @ 17mm pitch)
  - LEG NOTCHES cut the flange (segmented extrusion as in V2)
  - LIP NOTCHES at end of webs
  - SWAGE depressions on web face
"""
...
<html><head>
<meta charset="utf-8">
<title>Truss 3D V3 - WEB HOLES at every intersection</title>
<style>
  body { margin: 0; overflow: hidden; font-family: Segoe UI, Arial, sans-serif; background: #1a202c; }
  #info { position: absolute; top: 12px; left: 12px; color: white; padding: 12px 16px; background: rgba(0,0,0,0.6); border-radius: 6px; font-size: 13px; max-width: 380px; pointer-events: none; }
...
<body>
<div id="info">
  <h1>Truss frame ''' + frame + ''' - 3D V3</h1>
  <p><b>Drag</b> rotate · <b>Right-drag</b> pan · <b>Scroll</b> zoom</p>
  <p><span class="key">WEB HOLES</span>: 3 black through-drilled holes at every junction (3×Ø3.8mm, 17mm pitch on web ℄)</p>
  <p><span class="key">LEG NOTCH</span>: flange cut away (segmented profile)</p>
  <p><span class="key">SWAGE</span>: pressed depression on web</p>
</div>
<div id="stats"></div>
<div class="controls">
  ...
  <div class="legend">
    Black drilled-through holes mark every WEB HOLE position. Toggle screws off to see the holes clearly.
  </div>
</div>
...
// =================
// WEB HOLES + screws at every centreline crossing node
// 3 actual through-drilled holes at each node
// =================
DATA.nodes.forEach(node => {
  const cx = node.pt[0], cz = node.pt[1];
  const cy = DATA.centre[1];

  // Find web direction at this node
  let webStick = null;
  ...
  // Place 3 WEB HOLES at -17, 0, +17 along the perpendicular
  for (const off of [-17, 0, 17]) {
    const hx = cx + perpX*off;
    const hz = cz + perpZ*off;

    // 1. The actual drilled hole - solid black cylinder piercing through
    // Cylinder oriented along Y (vertical) since members lie in horizontal plane
    // Make it tall enough to clearly pierce both members at the junction
    const holeGeom = new THREE.CylinderGeometry(1.9, 1.9, 100, 16);
    const hole = new THREE.Mesh(holeGeom, holeMat);
    hole.position.set(hx, cy, hz);
    holesGroup.add(hole);

    // 2. Black ring on top to mark the hole entry (draws on top)
    const ringTopGeom = new THREE.RingGeometry(1.9, 2.4, 20);
    const ringTopMat = new THREE.MeshBasicMaterial({ color: 0x000000, side: THREE.DoubleSide });
    const ringTop = new THREE.Mesh(ringTopGeom, ringTopMat);
    ringTop.position.set(hx, cy + 50.5, hz);
    ringTop.rotation.x = -Math.PI/2;
    holesGroup.add(ringTop);

    // 3. Green-cap screw (sits in the hole)
...
out = os.path.join(OUT_DIR, 'truss_3d_v3.html')
open(out, 'w', encoding='utf-8').write(html)
print(f'Wrote V3 3D model: {out}')
print(f'  Members: {len(sticks)}')
print(f'  Nodes: {len(nodes)}')
print(f'  WEB HOLES: {len(nodes)*3} (3 per node)')
